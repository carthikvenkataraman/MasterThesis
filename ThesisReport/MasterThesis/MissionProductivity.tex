...
\chapter{Mission Productivity}
	...
	\section{Mission productivity definitions}
		...
		\begin{itemize}
			...
			where $N_{trips}$ \nomenclature{$N_{trips}$}{Average annual number of trips} is the average annual number of trips covered for the given mission, $M_{payload}$ \nomenclature{$M_{payload}$}{Average load carried per mission (kg)} is the average load carried per mission (kg) and $D_{annual}$ \nomenclature{$D_{annual}$}{Average total vehicle distance covered (km)} is the average total vehicle distance covered (km).
			...
			where $Cost_{operating}$ \nomenclature{$Cost_{operating}$}{Average annual operating cost} is the average annual operating cost. This may itself be expressed as 

			\begin{equation}
				Cost_{operating} = Cost_{fuel} + Cost_{personnel} + Cost_{maintenance} + Cost_{miscellaneous} \ \euro
			\end{equation}

			where $Cost_{fuel}$ \nomenclature{$Cost_{fuel}$}{Average annual fuel expense \euro} is the average annual fuel expense, $Cost_{personnel}$ \nomenclature{$Cost_{personnel}$}{Average annual personnel cost \euro} is the average annual cost incurred by employment of personnel including the driver, $Cost_{maintenance}$ \nomenclature{$Cost_{maintenance}$}{Average annual maintenance cost \euro} is the average annual cost of maintenance of the truck and associated operational elements and $Cost_{miscellaneous}$ \nomenclature{$Cost_{miscellaneous}$}{Average annual miscellaneous cost \euro} is the sum of all miscellaneous costs arising from the freight transport.\\
			...
			where $T_{driver}$ \nomenclature{$T_{driver}$}{Average annual number of driver hours} is the average annual number of driver hours billed to the freight transport company (including rest and halt hours).\\
			...
			where $V_{fuel}$ \nomenclature{$V_{fuel}$}{Average annual fuel consumption (l)} is the average annual fuel consumption in litres.\\
			...
			where $M_{emissions}$ \nomenclature{$M_{emissions}$}{Average annual carbon dioxide emission (mg)} is the average annual number of milligrams of carbon dioxide and other noxious pollutant emissions generated by the vehicle.
			...
			where $Revenue_{annual}$ \nomenclature{$Revenue_{annual}$}{Average annual revenue \euro} is the average annual revenue earned over a target duration of $N_{first\ owner}$ years \nomenclature{$N_{first\ owner}$}{Number of years of first ownership}, $Cost_{fixed}$ \nomenclature{$Cost_{fixed}$}{Initial investment in tractor \& trailers \euro} is the initial investment in the tractor, trailers and machinery and $Cost_{variable}$ \nomenclature{$Cost_{variable}$}{Total operating cost over $N_{first\ owner}$ years \euro} is the total operating cost over $N_{first\ owner}$ years.
	...
	\section{Productivity formulation}
		...
		The average revenue earned per unit freight carried, $R_{unit\ freight}$ \nomenclature{$R_{unit\ freight}$}{Revenue earned per unit freight \euro/tonne-km} \euro \ has been discussed in the earlier section and is used to arrive at annual mission revenues.

		\begin{equation}
			R_{mission} = R_{unit\ freight} \times M_{payload,\ net} \times D_{mission} \  \euro / mission
		\end{equation}
		where, $R_{mission}$ \nomenclature{$R_{mission}$}{Revenue earned per mission \euro} is the revenue earned per mission (\euro). The net payload carried $M_{payload,\ net}$ is expressed as:

		\begin{equation}
			M_{payload,\ net} = M_{payload,\ gross} - \Delta M_{eAxle}
		\end{equation}
		where the total mass due to propulsion of additional axles on trailers is $\Delta M_{eAxle}$ \ \euro \ \nomenclature{$\Delta M_{eAxle}$}{Total additional propulsion mass (kg)} and $M_{payload,\ gross}$ \nomenclature{$M_{payload,\ gross}$}{Payload carried by conventional combination (kg)} is the mass of the payload carried by the conventional combination. The total annual mission revenue earned by the freight transporter $R_{annual}$ is:

		\begin{equation}
			R_{annual} = R_{mission} \times N_{mission,\ annual} \  \euro / year
		\end{equation}
		where, $N_{mission,\ annual}$ \nomenclature{$N_{mission,\ annual}$}{Average annual number of trips} is the average annual number of trips made. \\

		The level of utilisation of the available axle load capacity and trailer volume is represented using a vehicle utilisation factor $U$. The combination is certified volume or tonnage limited based on if $V_{payload,\ max} \times \rho_{payload} \leq M_{axle, max}$ \nomenclature{$V_{payload,\ max}$}{Payload volume cbm} \nomenclature{$\rho_{payload}$}{Payload density kg.m\textsuperscript{-3}} \nomenclature{$M_{axle, max}$}{Payload density kg.m\textsuperscript{-3}} \nomenclature{$M_{axle, max}$}{Total axle load limit (kg)}. The utilisation factor is then defined as below:
		...
		\nomenclature{$R_{annual,\ corrected}$}{Corrected revenues \euro} The revenue earned over the target first owner duration of $N_{first\ owner}$ years is then calculated as:
		...
		\subsection{Fixed costs}
			...
			\end{equation}
			\nomenclature{$C_{fixed,\ conv}$}{Fixed cost of conventional combination \euro} \nomenclature{$N_{units}$}{Number of units in the combination} \nomenclature{$C_{fixed,\ elec, i}$}{Fixed cost of additional propulsion \euro} where, 

			\begin{equation}
				C_{fixed,\ conv} = P_{tractor,\ net}+P_{dolly}+P_{ST,1}+P_{ST,2}
			\end{equation}

			\nomenclature{$P_{tractor,\ net}$}{Net tractor price \euro} \nomenclature{$P_{dolly}$}{Dolly price \euro} \nomenclature{$P_{ST,i}$}{Price of semitrailer $\textit{i}$ \euro} Premiums on higher powertrains $\Delta P_{powertrain}$ \nomenclature{$P_{ST,i}$}{Powertrain premium (tractor) \euro} are added as:
			
			\begin{equation}
				P_{tractor,\ net} = P_{tractor,\ base} + \Delta P_{powertrain}
			\end{equation}
			where $P_{tractor,\ base}$ \nomenclature{$P_{tractor,\ base}$}{Base tractor price (D11) \euro} is the price of the tractor with the base powertrain configuration. 

			\begin{equation}
				C_{fixed,\ elec, i} = P_{batt,i}+\displaystyle \sum_{j=1}^{N_{eAxle,i}} P_{eAxle,j}
			\end{equation}
			\nomenclature{$P_{batt,i}$}{Buffer price over the first-owner lifetime of the tractor, unit i \euro} \nomenclature{$N_{eAxle,i}$}{Number of propelled axles on specified unit} \nomenclature{$P_{eAxle,j}$}{Price of electrification of a single axle \euro} The battery price for a single unit i over the first-owner lifetime of the tractor, $P_{batt,i}$, is expressed as
			\begin{equation}
				P_{batt,i}= S_{batt} \times P_{batt/kWh} \times N_{batt, replacement}
			\end{equation}
			\nomenclature{$S_{batt}$}{Battery size kWh} \nomenclature{$P_{batt/kWh}$}{Battery price per kWh} The price of the electrically propelled axle $P_{eAxle,j}$ consists of the electric machine, power electronics and transmission components, all of which are considered to be included in the electric motor prices discussed in the previous section. The total mass of electric axles, represented by $\Delta M_{eAxle}$ in the previous subsection can be expressed as:

			\begin{equation}
				\Delta M_{eAxle} = \displaystyle \sum_{i=2}^{N_{units}} M_{eAxle,i}
			\end{equation}
			\nomenclature{$M_{eAxle,i}$}{Mass of additional propulsion added, each unit} where, 
			\begin{equation}
				M_{eAxle,i} = (M_{battery,i} + \sum_{j=1}^{N_{eAxle,i}} M_{mot,j}) \forall i=2,..N_{units}
			\end{equation}
			\nomenclature{$M_{battery,i}$}{Battery mass, each unit}
		...
		\subsection{Operational Costs}
			...
			\end{equation}
			where the fuel costs $C_{fuel, litre}$ are the average EU-27 costs mentioned earlier. Maintenance costs $C_{mnt}$ are expressed in terms of a relative ratio to driver costs, $R_{mnt-driver}$. Ratios are as derived from Table \ref{table:OperatingCostContributors}. \nomenclature{$C_{fuel, litre}$}{Fuel price per litre} \nomenclature{$T_{mission}$}{Mission time hours} \nomenclature{$C_{driver, hour}$}{Driver hourly wages \euro / hour} \nomenclature{$C_{tyres}$}{Tyre maintenance costs per mission \euro} \nomenclature{$C_{fuel}$}{Fuel costs per mission \euro} \nomenclature{$C_{fuel}$}{Driver costs per mission \euro} \nomenclature{$C_{mnt}$}{Maintenance costs per mission \euro} \nomenclature{$C_{mnt}$}{Tolls per mission \euro} \nomenclature{$C_{elec}$}{Electricity costs (for plugin recharge) per mission \euro}
